...
for(const entry of ['index.html','photos.html','src','assets'])fs.cpSync(path.join(root,entry),path.join(dist,entry),{recursive:true,filter:source=>path.relative(root,source)!==path.join('assets','audio')});
